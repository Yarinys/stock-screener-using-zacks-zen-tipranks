...
TIPRANKS_CSV_SCORES: dict[str, int] = {}
TIPRANKS_CSV_ONLY = False
TIPRANKS_LIVE_DISABLED = False
TIPRANKS_DISABLE_ON_403 = True
TIPRANKS_LOCK = threading.Lock()
TIPRANKS_NEEDED: set[str] = set()
TIPRANKS_NEEDED_LOCK = threading.Lock()


def parse_provider_score_value(raw: object, provider: str) -> Optional[int]:
    """Parse a provider score value from CSV text. Currently used for TipRanks."""
    if raw is None:
        return None
    ...
    # Accept values like: 10, "10", "10/10", "Smart Score 10".
    match = re.search(r"\b(10|[1-9])\b", text)
    if not match:
        raise ValueError(f"Bad {provider} score value: {raw!r}")

    value = int(match.group(1))
    if provider == "tipranks" and not (1 <= value <= 10):
        raise ValueError(f"TipRanks score must be 1..10, got {value}.")
    return value


def load_tipranks_csv(file_path: Optional[str]) -> dict[str, int]:
    """Load TipRanks scores from CSV.

    Supported columns:
      ticker,tip_ranks
      ticker,tipranks_score
      symbol,smart_score
      symbol,score
    """
    if not file_path:
    ...
    if not path.exists():
        raise FileNotFoundError(f"TipRanks CSV not found: {path}")

    scores: dict[str, int] = {}
    with path.open("r", encoding="utf-8-sig", newline="") as f:
        reader = csv.DictReader(f)
        if not reader.fieldnames:
            raise ValueError(f"TipRanks CSV has no header row: {path}")

        field_map = {name.lower().strip(): name for name in reader.fieldnames}
        ticker_col = None
        for candidate in ["ticker", "symbol", "stock"]:
            if candidate in field_map:
                ticker_col = field_map[candidate]
                break
        if ticker_col is None:
            raise ValueError("TipRanks CSV must have a ticker/symbol/stock column.")

        score_col = None
        for candidate in ["tip_ranks", "tipranks", "tipranks_score", "smart_score", "smartscore", "score"]:
            if candidate in field_map:
                score_col = field_map[candidate]
                break
        if score_col is None:
            raise ValueError("TipRanks CSV must have a tip_ranks/tipranks_score/smart_score/score column.")

        for row in reader:
            ticker = str(row.get(ticker_col, "")).strip().upper().lstrip("$")
            if not ticker:
                continue
            score = parse_provider_score_value(row.get(score_col), "tipranks")
...
def configure_tipranks_csv(
    file_path: Optional[str],
    csv_only: bool,
    disable_on_403: bool,
) -> None:
    global TIPRANKS_CSV_SCORES, TIPRANKS_CSV_ONLY, TIPRANKS_DISABLE_ON_403
    TIPRANKS_CSV_SCORES = load_tipranks_csv(file_path)
    TIPRANKS_CSV_ONLY = bool(csv_only)
    TIPRANKS_DISABLE_ON_403 = bool(disable_on_403)


def note_tipranks_needed(ticker: str) -> None:
    with TIPRANKS_NEEDED_LOCK:
        TIPRANKS_NEEDED.add(ticker.upper().strip().lstrip("$"))


def write_tipranks_needed_csv() -> Optional[Path]:
    with TIPRANKS_NEEDED_LOCK:
        tickers = sorted(t for t in TIPRANKS_NEEDED if t)
    if not tickers:
        return None

    output_path = Path(__file__).resolve().parent / "tipranks_needed.csv"
...
def mark_tipranks_live_disabled(reason: str) -> None:
    global TIPRANKS_LIVE_DISABLED
    with TIPRANKS_LOCK:
        if not TIPRANKS_LIVE_DISABLED:
            TIPRANKS_LIVE_DISABLED = True
            print(f"TipRanks live requests disabled for this run: {reason}", file=sys.stderr)
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            if 400 <= response.status_code < 500:
                raise NonRetryableHTTPError(f"HTTP {response.status_code} for {url}", response.status_code, url)
...
def extract_tipranks_score_from_html(html: str) -> Optional[int]:
    """Extract TipRanks Smart Score from visible text or embedded page data."""
    text = html_to_text(html)

    # Visible-page patterns. Examples include:
    #   "Stock Smart Score 10"
    #   "Nvidia Stock Smart Score 10"
    visible_patterns = [
        r"Stock\s+Smart\s+Score\D{0,80}(10|[1-9])\b",
        r"Smart\s+Score\D{0,80}(10|[1-9])\b",
    ]
    ...
    # Embedded JSON/state-data patterns.
    json_patterns = [
...
def get_tipranks_score(ticker: str) -> tuple[int, str]:
    ...
    if ticker in TIPRANKS_CSV_SCORES:
        return TIPRANKS_CSV_SCORES[ticker], "tipranks_csv"

    if TIPRANKS_CSV_ONLY:
        note_tipranks_needed(ticker)
        raise RuntimeError(f"TipRanks score for {ticker} is not in --tipranks-csv and --tipranks-csv-only is enabled.")

    with TIPRANKS_LOCK:
        live_disabled = TIPRANKS_LIVE_DISABLED
    if live_disabled:
        note_tipranks_needed(ticker)
        raise RuntimeError("TipRanks live requests are disabled for this run after an earlier 403.")
    ...
    try:
        ...
        if exc.status_code == 403 and TIPRANKS_DISABLE_ON_403:
            note_tipranks_needed(ticker)
            mark_tipranks_live_disabled(
                "received HTTP 403 from TipRanks. Use --tipranks-csv/--tipranks-csv-only on GitHub Actions."
            )
        raise

    score = extract_tipranks_score_from_html(response.text)
    if score is not None:
        return score, url
...
def build_tipranks_cache(
    tickers: list[str],
    workers: int,
    progress_every: int,
    output_path: str,
) -> tuple[Path, list[str]]:
    """Fetch only TipRanks scores and write ticker,tip_ranks CSV."""
    path = Path(output_path.strip().strip('\"').strip("'"))
    ...
    def run_one(ticker: str) -> tuple[Optional[tuple[str, int]], Optional[str]]:
        ticker = ticker.upper().strip().lstrip("$")
        try:
            score, _url = get_tipranks_score(ticker)
    ...
    if workers <= 1:
        for index, ticker in enumerate(tickers, start=1):
            row, error = run_one(ticker)
            if row is not None:
                rows.append(row)
            if error is not None:
                errors.append(error)
            if progress_every > 0 and index % progress_every == 0:
                print(f"TipRanks cache progress: {index}/{total}", file=sys.stderr)
    else:
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for completed, future in enumerate(as_completed(future_to_ticker), start=1):
                ...
                if progress_every > 0 and completed % progress_every == 0:
                    print(f"TipRanks cache progress: {completed}/{total}", file=sys.stderr)
...
def score_many(
    ...
    def run_one(ticker: str) -> tuple[Optional[RatingResult], Optional[str]]:
        try:
            return score_ticker(ticker, exchange, zen_exchanges, skip_zen, skip_tipranks), None
        except Exception as exc:
            return None, f"{ticker}: {exc}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--skip-tipranks",
        action="store_true",
        help="Do not query TipRanks. TipRanks is treated as missing and the usual missing-TipRanks penalty applies.",
    )
    parser.add_argument(
        "--tipranks-csv",
        help=(
            "Optional CSV file with saved TipRanks scores. Supported columns: "
            "ticker,tip_ranks or ticker,tipranks_score or symbol,smart_score."
        ),
    )
    parser.add_argument(
        "--tipranks-csv-only",
        action="store_true",
        help="Use only --tipranks-csv for TipRanks. Never request tipranks.com live.",
    )
    parser.add_argument(
        "--write-tipranks-needed",
        action="store_true",
        help="Write tipranks_needed.csv listing tickers that still need TipRanks values.",
    )
    parser.add_argument(
        "--build-tipranks-cache",
        action="store_true",
        help="Only fetch TipRanks scores and write a local cache CSV, then exit. Useful to run locally if GitHub gets HTTP 403.",
    )
    parser.add_argument(
        "--tipranks-cache-output",
        default="tipranks_cache.csv",
        help="Output CSV path for --build-tipranks-cache. Default: tipranks_cache.csv.",
    )
    parser.add_argument(
        "--keep-trying-tipranks-after-403",
        action="store_true",
        help="Do not disable TipRanks live requests after the first HTTP 403. Not recommended on GitHub Actions.",
    )
    parser.add_argument(
        "--zen-top-candidates",
        type=int,
    ...
    if args.skip_tipranks and args.tipranks_csv_only:
        print("ERROR: Use either --skip-tipranks or --tipranks-csv-only, not both.", file=sys.stderr)
        return 1
    ...
    try:
        ...
        configure_tipranks_csv(
            file_path=args.tipranks_csv,
            csv_only=args.tipranks_csv_only,
            disable_on_403=not args.keep_trying_tipranks_after_403,
        )
    except Exception as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        return 1
    ...
    if args.build_tipranks_cache:
        cache_path, cache_errors = build_tipranks_cache(
            tickers=tickers,
            workers=args.workers,
            progress_every=args.progress_every,
            output_path=args.tipranks_cache_output,
        )
        print(f"Saved TipRanks cache CSV: {cache_path}", file=sys.stderr)
        needed_path = write_tipranks_needed_csv()
        if needed_path is not None:
            print(f"Saved TipRanks-needed CSV: {needed_path}", file=sys.stderr)
        for error in cache_errors:
            print(f"TIPRANKS CACHE ERROR: {error}", file=sys.stderr)
        return 0
    ...
    if args.zen_top_candidates is not None:
        rough_results, errors = score_many(
            ...
            args.progress_every,
            skip_zen=True,
            skip_tipranks=args.skip_tipranks,
        )
    ...
    if args.write_tipranks_needed:
        needed_path = write_tipranks_needed_csv()
        if needed_path is not None:
            print(f"Saved TipRanks-needed CSV: {needed_path}", file=sys.stderr)
        else:
            print("No missing TipRanks values to write.", file=sys.stderr)
